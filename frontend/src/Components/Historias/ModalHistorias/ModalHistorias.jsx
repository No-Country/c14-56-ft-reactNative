...
const ModalHistorias = ({ mi_modal, imageUrls, imageLength, actualIndex, users }) => {
  const [currentIndex, setCurrentIndex] = useState(0);

  const takeImages = imageUrls
  const takeUsers = users
  const defaultImageUrl = 'https://previews.123rf.com/images/pandavector/pandavector1709/pandavector170906212/86095866-persona-%C3%BAnico-icono-de-estilo-de-contorno-persona-ilustraci%C3%B3n-de-stock-de-s%C3%ADmbolo-del-vector.jpg'
              ...
              {allValues.map((imageUrl, index) => (
                ...
                >
                  <div className=''>
                    {allUsers[index] ? (
                      <div className='flex w-full h-1/2 absolute nose ' >
                        ...
                        />
                        <p className='mt-3 ml-2 text-slate-100'>{allUsers[index].name}</p>
                      </div>
                    ) : (
                      ''
                    )}
                  ...
                  <div className='h-full bg-slate-300 '>
                    <img src={allUsers[index] ? allUsers[index].storie : defaultImageUrl} className='object-cover h-full w' />
